...
#include "map_gen_BSP.h"
#include <iostream>

int node_count         = 0;

void map_gen_base_BSP(Map* map_pointer)
{
    node_count = 0;
...
void map_gen_BSP_split(Map* map_pointer, MapNode *map_node)
{
    ...
    if ((ROOM_MAX > 0) && (node_count >= ROOM_MAX))
    {
        split_x = false;
        split_y = false;
    }
...
void map_gen_BSP_internal(Map* map_pointer)
{
    ...
    for (int i =0; i < map_pointer->size(); i++)
    {
        temp_map->data.tile[i] = map_pointer->tile[i];
    }
    ...
    for (int i =0; i < map_pointer->size(); i++)
    {
        map_pointer->tile[i].data = temp_map->data.tile[i].data;
    }
